...
export default function SettingsView({ initialSettings = [] }: { initialSettings?: any[] }) {
  ...
  const settingsMap = initialSettings.reduce((acc, s) => ({ ...acc, [s.key]: s.value }), {
    agency_name: 'ProServices Digital',
    admin_email: 'admin@proservices.dz',
    agency_bio: 'Solutions digitales premium basées en Algérie.',
    brand_color: '#D4FF00'
  })
  ...
  const handleSaveColor = (color: string) => {
    setFormData((prev: any) => ({ ...prev, brand_color: color }))
    startTransition(async () => {
      await updateSettingAction('brand_color', color)
    })
  }
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input type="text" value={formData.agency_name} onChange={(e) => setFormData((prev: any) => ({ ...prev, agency_name: e.target.value }))} className="w-full bg-white/3 border border-white/5 rounded-xl py-4 px-6 text-sm text-white focus:border-brand/50 transition-all outline-none" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em] ml-2">Email Administratif</label>
                  <input type="email" value={formData.admin_email} onChange={(e) => setFormData((prev: any) => ({ ...prev, admin_email: e.target.value }))} className="w-full bg-white/3 border border-white/5 rounded-xl py-4 px-6 text-sm text-white focus:border-brand/50 transition-all outline-none" />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em] ml-2">Bio de l'Agence</label>
                <textarea rows={3} value={formData.agency_bio} onChange={(e) => setFormData((prev: any) => ({ ...prev, agency_bio: e.target.value }))} className="w-full bg-white/3 border border-white/5 rounded-xl py-4 px-6 text-sm text-white focus:border-brand/50 transition-all outline-none resize-none" />
              </div>
          ...
          {activeSection === 'appearance' && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="space-y-4">
                    <label className="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em] ml-2">Couleur Principale (Brand)</label>
                    <div className="flex flex-wrap gap-3 bg-white/3 border border-white/5 p-4 rounded-2xl">
                       {['#D4FF00', '#3b82f6', '#a855f7', '#ef4444', '#10b981', '#f59e0b', '#ec4899', '#06b6d4', '#8b5cf6', '#14b8a6', '#f43f5e', '#ffffff'].map(color => (
                          <button onClick={() => handleSaveColor(color)} key={color} className={`w-8 h-8 rounded-full flex items-center justify-center transition-transform hover:scale-110 ${color === formData.brand_color ? 'ring-2 ring-white ring-offset-2 ring-offset-[#09090b]' : ''}`} style={{ backgroundColor: color }}>
                             {color === formData.brand_color && <div className="w-2 h-2 bg-black rounded-full" />}
                          </button>
                       ))}
                    </div>
                 </div>
