...
const Footer = ({ dbSettings = [] }: { dbSettings?: any[] }) => {
  ...
  const contactAddress = getSetting('contact_address', 'Alger, Algérie');
  const officeHours = getSetting('office_hours', '');
  const copyrightText = getSetting('copyright_text', t("footer.rights"));
  ...
  const isVisible = (key: string) => getSetting(key, 'true') === 'true';
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-16">
          <div className="md:col-span-5">
            ...
            {isVisible('show_footer_desc') && (
              <p className="text-text-sub text-sm leading-relaxed mb-8 max-w-md">
                {footerDesc}
              </p>
            )}
            {officeHours && (
              <div className="flex items-center gap-2 text-[10px] text-brand/60 font-bold uppercase tracking-widest">
          ...
          <div className="md:col-span-3">
            <ul className="space-y-4">
              ...
              {isVisible('show_contact_address') && (
                <li className="flex items-start gap-3">
                  <div className="mt-0.5 text-brand">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                  </div>
                  <span className="text-text-sub text-sm">{contactAddress}</span>
                </li>
              )}
            </ul>
          </div>
        ...
        <div className="border-t border-border pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-4">
            {isVisible('show_social_fb') && (
              <a href={socialFb} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-full bg-surface border border-border flex items-center justify-center text-text-sub hover:text-brand transition-all">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"></path></svg>
              </a>
            )}
            {isVisible('show_social_ig') && (
              <a href={socialIg} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-full bg-surface border border-border flex items-center justify-center text-text-sub hover:text-brand transition-all">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
              </a>
            )}
            {isVisible('show_social_li') && (
              <a href={socialLi} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-full bg-surface border border-border flex items-center justify-center text-text-sub hover:text-brand transition-all">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path><rect x="2" y="9" width="4" height="12"></rect><circle cx="4" cy="4" r="2"></circle></svg>
              </a>
            )}
          </div>
          <p className="text-muted text-sm font-medium">
            © {new Date().getFullYear()} {agencyName}. {copyrightText}
          </p>
        </div>
      </div>
    </footer>
